...
**PATHS:** после подстановки — `$ARIA_DOCS/PATHS.yaml`.
...
ls /tmp/aria-upstream/scripts/             # validate_spec_e2e.sh, validate_e2e_results.py, validate_e2e_results.sh, hooks/
```
...
1. `CLAUDE.md.template` → `CLAUDE.md` — подставить `ARIA`, `AI-powered development framework for Claude Code`, `ARIA_DOCS`, `ARIA_CODE`, `{{ADAPTER_NAME}}`, `Bash, Markdown, YAML; Git + gh CLI + Claude Code Opus 4.6`, `[CORE] [ADAPTER] [PROTOCOL] [POLICY] [DOCS] [HOOKS] [FIX] [META]`, `core/ adapters/ scripts/ .github/`, `{{LANGUAGE_SPECIFIC_ANTIPATTERNS}}`, `{{INFRASTRUCTURE_SECTION}}`, `{{ARIA_VERSION}}`
...
3. `project_config.yaml.template` → `project_config.yaml` — **включая секцию `commands:` (R6, Task 1)**:
   ...
   commands:
     ...
     dev_server: "{из ШАГ 1.6или пустая строка}"
   aria:
...
cp /tmp/aria-upstream/core/commands/*.md "$DOCS_DIRECTORY/.claude/commands/"
# Также скопировать settings.local.json шаблон если есть
```
...
git add core/ adapters/ scripts/ .github/ scripts/hooks/ .mcp.json .gitignore
