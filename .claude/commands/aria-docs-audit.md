...
Для каждого `.md` в `$ARIA_DOCS/docs/` и `$ARIA_DOCS/*.yaml|*.md` в корне:
...
В `$ARIA_CODE` (code-repo) НЕ должно быть ни одного из запрещённых путей:
...
## Итог
SYNC — документация актуальна, все контуры замкнуты, разделение репо соблюдено.
```
