...
**PATHS:** Прочитай ${{ENV_DOCS}}/PATHS.yaml.
...
# Коммит релиза
git -C "${{ENV_CODE}}" add CHANGELOG.md
git -C "${{ENV_CODE}}" commit -m "[META] release v{X.Y.Z}: {заголовок}

Co-Authored-By: Claude Opus 4.6 (1M context) <noreply@anthropic.com>"

# Тег
git -C "${{ENV_CODE}}" tag -a "v{X.Y.Z}" -m "{заголовок}"

# Push + push tag
git -C "${{ENV_CODE}}" push origin main
git -C "${{ENV_CODE}}" push origin "v{X.Y.Z}"
```
...
**Проверка покрытия Repo (Task 2 аудита):**
Посчитай долю форков БЕЗ Repo (`—`) среди активных форков.
Если >50% без Repo — **WARN пользователю:**
```
ПРЕДУПРЕЖДЕНИЕ: {N} из {M} активных форков ({%}%) не имеют заполненного Repo.
Эти форки НЕ получат автоматическое уведомление о релизе.
Рекомендация: попросить владельцев форков указать Repo через /aria-sync --contribute или при следующем PR.
Форки без Repo: {список проектов}
```
